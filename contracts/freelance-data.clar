...
;; Global counters
(define-data-var category-counter uint)
(define-data-var user-counter uint)
...
(define-read-only (get-leaderboard-score (user principal) (score-type (string-ascii 20)))
  (begin
    (match (map-get? leaderboard {user: user, score-type: score-type})
      score (ok score)
      (err ERR-NOT-FOUND)
    )
  )
...
(define-public (add-category 
  ...
  (begin
    ...
    (if (is-eq tx-sender 'ST1SJ3DTEQDN9XJYV8KHGXG4M0DCE0P6Z2)
      (begin
...
(define-public (update-leaderboard-score (score-type (string-ascii 20)) (score-value uint))
  (begin
    (map-set leaderboard {user: tx-sender, score-type: score-type} {
...
(define-public (mint-achievement (achievement-type (string-ascii 20)) (token-id uint))
  (begin
    ...
    (match (map-get? achievements tx-sender)
      user-achievements
        (begin
          ...
          (match (get achievement-type user-achievements)
            existing-token (err ERR-ALREADY-EXISTS)
            (begin
              ;; Add new achievement
